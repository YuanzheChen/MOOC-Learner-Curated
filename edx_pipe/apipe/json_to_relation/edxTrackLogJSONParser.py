...
class EdXTrackLogJSONParser(GenericJSONParser):
    ...
    problemGradedComplexPattern = re.compile(
        r'aria-describedby=[\\"]*(input[^\\">]*)[\\"]*[>nStatus\\:\s"]*([iIn]{0,2}correct)'
    )
    ...
    def __init__(self,
        ...
        '''
        super(EdXTrackLogJSONParser, self).__init__(
            jsonToRelationConverter,
            logfileID=logfileID,
        ...
        self.commonFldNames = [
            'agent', 'event_source', 'event_type', 'ip', 'page', 'session',
            'time', 'username', 'course_id', 'course_display_name', 'context'
        ]
        ...
        self.hashMapper = ModulestoreImporter(
            os.path.join(
                os.path.dirname(__file__), 'data/modulestore_latest.json'),
        ...
        self.courseNamesSorted = sorted(
            self.hashMapper.keys(), key=len, reverse=True)

        self.schemaHintsMainTable = OrderedDict()

        self.schemaHintsMainTable['_id'] = ColDataType.UUID
        self.schemaHintsMainTable[
            'event_id'] = ColDataType.UUID  # we generate this one ourselves; xlates to VARCHAR(40). Not unique!
        ...
        self.schemaHintsMainTable[
            'resource_display_name'] = ColDataType.TINYTEXT
        ...
        self.schemaHintsMainTable[
            'long_answer'] = ColDataType.TEXT  # essay answers
        ...
        self.schemaHintsMainTable[
            'feedback_response_selected'] = ColDataType.TINYINT
        ...
        # Book (PDF) reading:
        self.schemaHintsMainTable[
            'book_interaction_type'] = ColDataType.TINYTEXT
        ...
        self.schemaHintsMainTable[
            'group_action'] = ColDataType.TINYTEXT  #'add', 'remove'; called 'event' in JSON

        # ajax
        self.schemaHintsMainTable[
            'position'] = ColDataType.INT  # used in event ajax goto_position
        ...
        for colName in self.schemaHintsMainTable.keys():
            ...
            self.schemaHintsMainTable[colName] = ColumnSpec(
                colName, colType, self.jsonToRelationConverter)
        ...
        for colName in self.schemaStateTbl.keys():
            ...
            self.schemaStateTbl[colName] = ColumnSpec(
                colName, colType, self.jsonToRelationConverter)
        ...
        for colName in self.schemaAnswerTbl.keys():
            ...
            self.schemaAnswerTbl[colName] = ColumnSpec(
                colName, colType, self.jsonToRelationConverter)
        ...
        for colName in self.schemaCorrectMapTbl.keys():
            ...
            self.schemaCorrectMapTbl[colName] = ColumnSpec(
                colName, colType, self.jsonToRelationConverter)
        ...
        for colName in self.schemaInputStateTbl.keys():
            ...
            self.schemaInputStateTbl[colName] = ColumnSpec(
                colName, colType, self.jsonToRelationConverter)
        ...
        self.schemaEventIpTbl[
            'event_ip'] = ColDataType.TINYTEXT  # IP address of respective event
        # Turn the SQL data types in the dict to column spec objects:
        for colName in self.schemaEventIpTbl.keys():
            colType = self.schemaEventIpTbl[colName]
            self.schemaEventIpTbl[colName] = ColumnSpec(
                colName, colType, self.jsonToRelationConverter)
        ...
        for colName in self.schemaABExperimentTbl.keys():
            ...
            self.schemaABExperimentTbl[colName] = ColumnSpec(
                colName, colType, self.jsonToRelationConverter)
        ...
        self.schemaAccountTbl[
            'screen_name'] = ColDataType.TEXT  # chosen screen name
        ...
        self.schemaAccountTbl[
            'zipcode'] = ColDataType.TINYTEXT  # Picked out from mailing_address
        self.schemaAccountTbl[
            'country'] = ColDataType.TINYTEXT  # Picked out from mailing_address
        ...
        for colName in self.schemaAccountTbl.keys():
            ...
            self.schemaAccountTbl[colName] = ColumnSpec(
                colName, colType, self.jsonToRelationConverter)
        ...
        for colName in self.schemaLoadInfoTbl.keys():
            ...
            self.schemaLoadInfoTbl[colName] = ColumnSpec(
                colName, colType, self.jsonToRelationConverter)
        ...
        loadInfoDict[
            'load_date_time'] = self.jsonToRelationConverter.loadDateTime
        ...
        try:
            ...
            try:
                ...
                if eventType is None:
                    raise KeyError(
                        "No event type field; cannot determine tye type of event."
                    )
            ...
            try:
                ...
                # Modified the time stamp for new MITx logs
                # TODO: make it compatible for all logs later
                eventDateTime = datetime.datetime.strptime(
                    eventTimeStr, '%Y-%m-%d %H:%M:%S.%f %Z')
            except KeyError:
            ...
            try:
                ...
                self.setValInRow(row, 'downtime_for',
            ...
            try:
                ...
                raise ValueError("Event of type %s has no event field" %
                                 eventType)

            try:
                if '{' not in eventJSONStrOrDict or eventJSONStrOrDict[:
                                                                       5] == 'input':
                    event = eventJSONStrOrDict  # pass along simple case of problem_check/reset/save event (not JSON, just an ordinary string)
                else:
                ...
                try:
                    ...
                    raise ValueError(
                        'Bad JSON; saved in col badlyFormatted: event_type %s (%s)'
                        % (eventType, ` e1 `))
                    return
            ...
            if eventType == 'seq_goto' or\
               ...
               eventType == 'seq_prev':
                ...
                row = self.handleQuestionProblemHidingShowing(record, row,
                                                              event)
                return
                ...
                return
            elif eventType == 'edx.video.closed_captions.hidden' or eventType == 'edx.video.closed_captions.shown':
                row = self.handleShowHideCaptions(record, row, event)
                return
            elif eventType == 'play_video' or\
                 eventType == 'pause_video' or\
                 eventType == 'load_video' or\
            ...
            elif eventType == 'video_hide_cc_menu' or\
                 eventType == 'video_show_cc_menu':
                row = self.handleVideoHideShowCCMenu(record, row, event)
                return
                ...
                row = self.handleSaveProblemFailSuccessCheckOrReset(record,
                                                                    row, event)
                return
            ...
            # Instructor events:
                    ...
                    'list-students', 'dump-grades', 'dump-grades-raw',
                    'dump-grades-csv', 'dump-grades-csv-raw',
                    'dump-answer-dist-csv', 'dump-graded-assignments-config',
                ...
                return
                    ...
                    'list-forum-admins', 'list-forum-mods',
                ...
                return
                    ...
                    'remove-forum-admin', 'add-forum-admin',
                    'remove-forum-mod', 'add-forum-mod',
                    'remove-forum-community-TA', 'add-forum-community-TA'
            ]:
            ...
            # Event type values that start with slash:
                ...
                self.logWarn("Unknown event type '%s' in tracklog row %s" %
                             (eventType,
            ...
            if row is not None and len(row) != 0 and not self.errorOccurred:
                self.jsonToRelationConverter.pushToTable(
    ...
    def resultTriplet(self, row, targetTableName, colNamesToSet=None):
        ...
        if colNamesToSet is None and targetTableName != self.mainTableName:
            raise ValueError(
                "If colNamesToSet is None, the target table must be the main table whose name was passed into __init__(); was %s"
                % targetTableName)
        ...
        if colNamesToSet is not None:
            ...
            return (targetTableName,
                    ','.join(self.colNamesByTable[targetTableName]), row)
    ...
    def pushTableCreations(self, replaceTables):
        ...
        if replaceTables:
            ...
            self.jsonToRelationConverter.pushString(
                'DROP TABLE IF EXISTS %s, Answer, InputState, CorrectMap, State, Account, EdxPrivate.Account, LoadInfo;\n'
                % self.mainTableName)
    ...
    def genOneCreateStatement(self,
                              tableName,
                              schemaDict,
                              primaryKeyName=None,
                              foreignKeyColNames=None,
                              autoincrement=False):
        '''
        ...
        if foreignKeyColNames is not None:
            for foreignTableName in foreignKeyColNames.keys():
                (localFldName,
                 foreignKeyName) = foreignKeyColNames[foreignTableName]
    ...
    def createAnswerTable(self):
        createStatement = self.genOneCreateStatement(
            'Answer', self.schemaAnswerTbl, primaryKeyName='answer_id')
        self.jsonToRelationConverter.pushString(createStatement)
        ...
        self.jsonToRelationConverter.startNewTable('Answer',
                                                   self.schemaAnswerTbl)
    ...
    def createCorrectMapTable(self):
        ...
        # a CSV file and CSV writer to which rows destined for this
        # table will be written:
        self.jsonToRelationConverter.startNewTable('CorrectMap',
                                                   self.schemaCorrectMapTbl)
    ...
    def createInputStateTable(self):
        ...
        # a CSV file and CSV writer to which rows destined for this
        # table will be written:
        self.jsonToRelationConverter.startNewTable('InputState',
                                                   self.schemaInputStateTbl)
    ...
    def createStateTable(self):
        ...
        # a CSV file and CSV writer to which rows destined for this
        # table will be written:
        self.jsonToRelationConverter.startNewTable('State',
                                                   self.schemaStateTbl)
    ...
    def createAccountTable(self):
        ...
        # process:
        createStatement = self.genOneCreateStatement(
            'Account', self.schemaAccountTbl, primaryKeyName='account_id')
        self.jsonToRelationConverter.pushString(createStatement)
        ...
        # a CSV file and CSV writer to which rows destined for this
        # table will be written:
        self.jsonToRelationConverter.startNewTable('Account',
                                                   self.schemaAccountTbl)
    ...
    def createEventIpTable(self):
        ...
        # process:
        createStatement = self.genOneCreateStatement(
            'EventIp', self.schemaEventIpTbl, primaryKeyName='event_table_id')
        self.jsonToRelationConverter.pushString(createStatement)
        createStatement = self.genOneCreateStatement(
            'EdxPrivate.EventIp',
            self.schemaEventIpTbl,
    ...
    def createABExperimentTable(self):
        ...
        # a CSV file and CSV writer to which rows destined for this
        # table will be written:
        self.jsonToRelationConverter.startNewTable('ABExperiment',
                                                   self.schemaABExperimentTbl)

    def createLoadInfoTable(self):
        createStatement = self.genOneCreateStatement(
            'LoadInfo', self.schemaLoadInfoTbl, primaryKeyName='load_info_id')
        self.jsonToRelationConverter.pushString(createStatement)
        # Tell the output module (output_disposition.OutputFile) that
        # it needs to know about a new table. That module will create
        # a CSV file and CSV writer to which rows destined for this
        # table will be written:
        self.jsonToRelationConverter.startNewTable('LoadInfo',
    ...
    def createMainTable(self):
        ...
        # a CSV file and CSV writer to which rows destined for this
        # table will be written:
        self.jsonToRelationConverter.startNewTable(self.mainTableName,
                                                   self.schemaHintsMainTable)
    ...
    def handleCommonFields(self, record, row):
        ...
        for fldName in self.commonFldNames:
            ...
            if fldName == 'event_type' and val is not None:
                ...
                (fullCourseName, course_id, displayName) = self.get_course_id(
                    record)  # @UnusedVariable
                val = course_id
                ...
                if self.currCourseDisplayName is not None:
                    ...
                    (fullCourseName, course_id,
                     displayName) = self.get_course_id(
                         record)  # @UnusedVariable
                    val = displayName
                ...
                if self.currContextDict is not None:
                    ...
                    abTestInfo = self.currContextDict.get('course_user_tags',
                                                          None)
                    if abTestInfo is not None:
                        ...
                        if abTestDictInfoDict is not None:
                            ...
                            # table:
                            self.handleABExperimentEvent(record, row,
                                                         abTestDictInfoDict)
    ...
    def handleSeqNav(self, record, row, event, eventType):
        ...
        try:
            ...
            self.logWarn(
                "Track log line %s with event type %s is missing sequence id" %
    ...
    def handleProblemCheck(self, record, row, event):
        ...
        while not generatedAllRows:
            ...
            self.setValInRow(row, 'correctMap_fk', correctMapFKey
                             if correctMapFKey is not None else '')
            self.setValInRow(row, 'answer_fk', answerFKey
                             if answerFKey is not None else '')
            if answerFKey is not None:
                # For convenience: enter the Answer's problem ID 
                # in the main table's problemID field:
                problemID = answerToProblemMap[
                    answerFKey] if answerToProblemMap[
                        answerFKey] is not None else ''
                self.setValInRow(row, 'problem_id', problemID)
            self.setValInRow(row, 'state_fk', stateFKey
                             if stateFKey is not None else '')
    ...
    def pushCorrectMaps(self, correctMapsDict):
        ...
        for answerKey in correctMapsDict.keys():
            ...
            correctMapValues = [
                correct_map_id, answer_id, correctness, npoints, msg, hint,
                mode, queuestate
            ]
            self.jsonToRelationConverter.pushToTable(
                self.resultTriplet(correctMapValues, 'CorrectMap',
                                   self.schemaCorrectMapTbl.keys()))
    ...
    def pushAnswers(self, answersDict):
        ...
        for problemID in answersDict.keys():
            ...
            if answer is not None:
                ...
                answerValues = [
                    answersKey,  # answer_id fld 
                    problemID,  # problem_id fld
                    answer,
                    self.currCourseID
                ]
                self.jsonToRelationConverter.pushToTable(
                    self.resultTriplet(answerValues, 'Answer',
                                       self.schemaAnswerTbl.keys()))
    ...
    def pushState(self, stateDict):
        ...
        if studentAnswersDict is not None:
            ...
            # to the problem ID to which that key's row
            # in the Answer refers:
            (studentAnswersFKeys, answerToProblemMap) = self.pushAnswers(
                studentAnswersDict)  # @UnusedVariable
        else:
            studentAnswersFKeys = []
        ...
        while not generatedAllRows:
            ...
            stateValues = [
                state_id, seed, done, problemID, studentAnswerFKey,
                correctMapFKey, inputStateFKey
            ]
            rowInfoTriplet = self.resultTriplet(stateValues, 'State',
                                                self.schemaStateTbl.keys())
            self.jsonToRelationConverter.pushToTable(rowInfoTriplet)
            indexToFKeys += 1
    ...
    def pushInputStates(self, inputStatesDict):
        ...
        for problemID in inputStatesDict.keys():
            ...
            if inputStateProbVal is not None:
                ...
                inputStateValues = [
                    inputStateKey, problemID, inputStateProbVal
                ]
                self.jsonToRelationConverter.pushToTable(
                    self.resultTriplet(inputStateValues, 'InputState',
                                       self.schemaInputStateTbl.keys()))
    ...
    def pushEventIpInfo(self, eventIpDict):
        ...
        '''
        self.jsonToRelationConverter.pushToTable(
            self.resultTriplet(eventIpDict.values(), 'EventIp',
    ...
    def pushABExperimentInfo(self, abExperimentDict):
        ...
        '''
        self.jsonToRelationConverter.pushToTable(
            self.resultTriplet(abExperimentDict.values(), 'ABExperiment',
                               self.schemaABExperimentTbl.keys()))
        return
    ...
    def pushAccountInfo(self, accountDict):
        ...
        self.jsonToRelationConverter.pushToTable(
            self.resultTriplet(accountDict.values(), 'Account',
                               self.schemaAccountTbl.keys()))
        return
    ...
    def pushLoadInfo(self, loadDict):
        ...
        self.jsonToRelationConverter.pushToTable(
            self.resultTriplet(loadDict.values(), 'LoadInfo',
                               self.schemaLoadInfoTbl.keys()))
    ...
    def handleProblemReset(self, record, row, event):
        ...
        if postGetDict is None:
            if isinstance(event, basestring):
                # It turns out that problem_reset events of the simple case described above
                # may appear more like the event string handled by handleProblemCheckSimpleCase.
                # So it should suffice to pass the event onto that handler to avoid
                # code duplication
                return self.handleProblemCheckSimpleCase(row, event)
            else:
        ...
        problemIDs = None  # Avoid UnboundLocalError: local variable 'problemIDs' referenced before assignment
        try:
            problemIDs = postGetDict['POST']['id']
        except KeyError:
            self.logWarn(
                "Track log line %s with event type problem_reset contains event without problem ID array: '%s'"
    ...
    def handleProblemShow(self, record, row, event):
        ...
        # Get the problem id:
        problemID = None  # Avoid UnboundLocalError: local variable 'problemID' referenced before assignment
        try:
            problemID = postGetDict['problem']
        except KeyError:
            self.logWarn(
                "Track log line %s with event type problem_show contains event without problem ID: '%s'"
                % (self.jsonToRelationConverter.makeFileCitation(), event))
            return row
        self.setValInRow(row, 'problem_id', problemID)
        # Try to look up the human readable display name
    ...
    def handleVideoPlayPause(self, record, row, event):
        ...
        videoCurrentTime = str(
            valsDict['currentTime']) if 'currentTime' in valsDict else None
        videoSpeed = str(valsDict['speed']) if 'speed' in valsDict else None
        ...
        self.setValInRow(row, 'video_speed', videoSpeed)
        return row
    ...
    def handleVideoSeek(self, record, row, event):
        ...
        videoCode = valsDict.get('code', None)
        videoOldTime = str(
            valsDict['old_time']) if 'old_time' in valsDict else None
        videoNewTime = str(
            valsDict['new_time']) if 'new_time' in valsDict else None
    ...
    def handleVideoSpeedChange(self, record, row, event):
        ...
        videoCurrentTime = str(
            valsDict['currentTime']) if 'currentTime' in valsDict else None
        videoOldSpeed = str(
            valsDict['old_speed']) if 'old_speed' in valsDict else None
        videoNewSpeed = str(
            valsDict['new_speed']) if 'new_speed' in valsDict else None
    ...
    def handleVideoHideShowCCMenu(self, record, row, event):
        '''
        video_hide_cc_menu Events look like this::

           "{\"id\":\"0b9e39477cf34507a7a48f74be381fdd\",
             \"code\":\"html5\",
             \"language\":\"english\"}"

        video_show_cc_menu Events look like this::
        ...
        '''
        if event is None:
            self.logWarn("Track log line %s: missing event text in video show or hide cc menu." %\
                         (self.jsonToRelationConverter.makeFileCitation()))
            return row

        valsDict = self.ensureDict(event)
        if valsDict is None:
            self.logWarn("Track log line %s: event is not a dict in video show or hide cc menu: '%s'" %\
                         (self.jsonToRelationConverter.makeFileCitation(), str(event)))
            return row
    ...
    def handleShowHideTranscript(self, record, row, event):
        ...
        ''' id and code member fields are really video_id and video_code
        Also, there exist currentTime field for these two events as described in edx doc
        '''

        videoID = event.get('id', None)
        videoCode = event.get('code', None)
        self.setValInRow(row, 'transcript_id', videoID)
        self.setValInRow(row, 'transcript_code', videoCode)

        self.setResourceDisplayName(row, videoID)
        self.setValInRow(row, 'video_id', videoID)
        self.setValInRow(row, 'video_code', videoCode)
        videoCurrentTime = str(
            event['currentTime']) if 'currentTime' in event else None
        self.setValInRow(row, 'video_current_time', videoCurrentTime)

        return row

    def handleShowHideCaptions(self, record, row, event):
        '''
        Events look like this::
        ...
        Same for edx.video.closed_captions.hidden
        ...
        '''Created to extend json2relation to handle edx.video.closed_captions.hidden
        and edx.video.closed_captions.shown. As they are included in edx doc
        '''
    ...
    def handleCreateAccount(self, record, row, event):
        ...
        if isinstance(mailAddr, list):
            mailAddr = mailAddr[0]
            accountDict = self.getZipAndCountryFromMailAddr(mailAddr,
                                                            accountDict)
        else:
            accountDict['zipcode'] = ''
            accountDict['country'] = ''
        ...
        try:
            accountDict['terms_of_service'] = 1 if accountDict[
                'terms_of_service'] == 'true' else 0
        except:
            pass

        try:
            accountDict['honor_code'] = 1 if accountDict[
                'honor_code'] == 'true' else 0
        except:
            pass
        ...
        if len(accountDict['screen_name']) > 0:
            accountDict['screen_name'] = self.makeInsertSafe(
                accountDict['screen_name'])
        if len(accountDict['name']) > 0:
            accountDict['name'] = self.makeInsertSafe(accountDict['name'])
        if len(accountDict['mailing_address']) > 0:
            accountDict['mailing_address'] = self.makeInsertSafe(
                accountDict['mailing_address'])
    ...
    def handleProblemGraded(self, record, row, event):
        ...
        # respectively:
        probIdCorrectIterator = EdXTrackLogJSONParser.problemGradedComplexPattern.finditer(
            str(event))
        ...
        if len(answersFKeys) > 0:
            ...
            for answerFKey in answersFKeys:
                ...
                if answerFKey is not None:
                    ...
                    rowInfoTriplet = self.resultTriplet(row,
                                                        self.mainTableName)
                    self.jsonToRelationConverter.pushToTable(rowInfoTriplet)
                    # The next row keeps its eventID, but needs its own
                    # primary key (in _id):
    ...
    def finish(self, includeCSVLoadCommands=False, outputDisposition=None):
        ...
        if includeCSVLoadCommands:
            self.jsonToRelationConverter.pushString(
                self.createCSVTableLoadCommands(outputDisposition))
    ...
    def createCSVTableLoadCommands(self, outputDisposition):
        ...
        for tableName in [
                'LoadInfo', 'InputState', 'State', 'CorrectMap', 'Answer',
                'Account', 'EdxTrackEvent'
        ]:
    ...
    def handleBadJSON(self, row, offendingText):
        ...
        :type offendingText:
        '''
        self.setValInRow(row, 'badly_formatted',
                         self.makeInsertSafe(offendingText))
        return row
    ...
    def get_course_id(self, event):
        ...
        if eventSource == 'server':
            ...
            if eventType == u'/accounts/login':
                ...
                return (courseID, courseID,
                        self.getCourseDisplayName(eventType))
        ...
        # Above logic makes an error for '/dashboard' events:
    ...
    def extractCourseIDFromProblemXEvent(self, event):
        ...
        # NOTE the other values can contain hyphes as well, e.g. the 'answers' for a 'submission'. Therefore we interpret the comments to explicitly look
        # in 'correct_map' for the course_id

        # Add for not parsed event field for the event type containing `problem_`
        # This is a bug fix for new MITx data
        if isinstance(event, unicode):
            event = json.loads(event)
        event = event.get('correct_map', None)
        if event is None:
            return None
    ...
    def makeInsertSafe(self, unsafeStr):
        ...
        if unsafeStr is None or not isinstance(
                unsafeStr, basestring) or len(unsafeStr) == 0:
            return ''
    ...
    def makeJSONSafe(self, jsonStr):
        ...
        :type jsonStr:
        '''
        res = EdXTrackLogJSONParser.JSON_BAD_BACKSLASH_PATTERN.sub(
            self.fixOneJSONBackslashProblem, jsonStr)
        return res
    ...
    def rescueBadJSON(self, badJSONStr, row=[]):
        ...
        :type badJSONStr:
        '''
        screen_name = self.tryJSONExtraction(
            EdXTrackLogJSONParser.searchPatternDict['username'], badJSONStr)
        #host = self.tryJSONExtraction(EdXTrackLogJSONParser.searchPatternDict['host'], badJSONStr)
        session = self.tryJSONExtraction(
            EdXTrackLogJSONParser.searchPatternDict['session'], badJSONStr)
        event_source = self.tryJSONExtraction(
            EdXTrackLogJSONParser.searchPatternDict['event_source'],
            badJSONStr)
        event_type = self.tryJSONExtraction(
            EdXTrackLogJSONParser.searchPatternDict['event_type'], badJSONStr)
        time = self.tryJSONExtraction(
            EdXTrackLogJSONParser.searchPatternDict['time'], badJSONStr)
        ip = self.tryJSONExtraction(
            EdXTrackLogJSONParser.searchPatternDict['ip'], badJSONStr)
        event = self.tryJSONExtraction(
            EdXTrackLogJSONParser.searchPatternDict['event'], badJSONStr)

        if isinstance(screen_name, basestring):
            self.setValInRow(row, 'anon_screen_name',
                             self.hashGeneral(screen_name))
        else:
            self.setValInRow(row, 'anon_screen_name', '')
    ...
    def getZipAndCountryFromMailAddr(self, mailAddr, accountDict):
        ...
        for mailWordIndx in range(len(splitMailAddr)):
            try:
                fourgram = string.join([
                    splitMailAddr[mailWordIndx],
            ...
            try:
                trigram = string.join([
                    splitMailAddr[mailWordIndx], splitMailAddr[mailWordIndx +
                                                               1],
                    splitMailAddr[mailWordIndx + 2]
                ])
    ...
    def setResourceDisplayName(self, row, openEdxHash):
        ...
        if openEdxHash is not None and len(openEdxHash) > 0:
            ...
            if displayName is not None:
                self.setValInRow(row, 'resource_display_name',
                                 self.makeInsertSafe(displayName))
